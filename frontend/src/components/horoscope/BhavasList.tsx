...
export const BhavasList = () => {
  ...
  const { bhavas, grahas } = horoscopeData;
  ...
  return (
    <div className="w-full">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {bhavas.map((bhava, index) => {
          const occupants = grahas
            .filter((g) => g.house_number === bhava.house_number)
            .map((g) => g.names.english);
          const flags = [
            bhava.kendra ? 'Kendra' : null,
            bhava.trikona ? 'Trikona' : null,
            bhava.dusthana ? 'Dusthana' : null,
                      ...
                      <div className="py-4">
                        ...
                        <div className="inline-block px-3 py-1 rounded-full bg-gold-primary/10 border border-gold-primary/20">
                          <p className="text-[11px] font-bold text-gold-light tracking-[0.15em] uppercase">
                            {bhava.rasi.tamil ?? bhava.rasi.english}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                <div className="absolute inset-0 [backface-visibility:hidden] [transform:rotateY(180deg)]">
                  <GlassCard className="h-full w-full p-6 flex flex-col justify-center items-center text-center border-gold-primary/20 bg-black/40">
                    ...
                    <div className="space-y-6">
                      <div>
                        <p className="text-gold-primary text-[11px] uppercase tracking-[0.3em] mb-3 font-bold">House Nature</p>
                        <div className="flex flex-wrap justify-center gap-2">
                          {flags.length > 0 ? (
                            flags.map((flag) => (
                              <span key={flag} className="text-[10px] px-2 py-0.5 rounded-md bg-gold-primary/10 border border-gold-primary/20 text-gold-light uppercase tracking-widest">
                                {flag}
                              </span>
                            ))
                          ) : (
                            <p className="text-white/90 text-sm leading-relaxed font-light">
                              No angular, trinal or dusthana classification.
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="pt-6 border-t border-white/10 w-full">
                        <p className="text-gold-light/60 text-[10px] uppercase tracking-widest mb-2 font-semibold">Sign & Lord</p>
                        <p className="text-gray-400 text-sm italic leading-relaxed px-2">
                          {bhava.rasi.english} sign, ruled by {bhava.lord}.
                        </p>
                      </div>
                    </div>
